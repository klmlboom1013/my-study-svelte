...
- WPAY 회원 가입 정보 조회 API URL.
  ...
  - Domain
    - WPAY 회원 가입 정보 조회 도메인은 secrets.ts의 ServiceType의 "wpaystd" 으로만 연동이 가능 하다.
      - 로그인 페이지에서 선택한 ServiceType을 "wpaystd"로 변경 하면 안된다.
...
- View 모달 하단에 API 연동이 성공이면 "확인" 실패면 "회원가입" 버튼을 표시 한다.
  - "확인" 버튼을 클릭하면 메인화면으로 이동 한다.
    - 상단의 "X" 버튼 클릭도 동일하게 동작 한다.
  - "회원가입" 버튼을 클릭하면 로그인 페이지에서 입력한 Cell Phone Number를 삭제하고 "3.1. 서비스 가입"을 수행한다.
    - 상단의 "X" 버튼 클릭하면 모달을 닫는다.